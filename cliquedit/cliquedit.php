<?php
require_once dirname(__FILE__).'/session.php';
require_once dirname(__FILE__).'/loader.php';
require_once dirname(__FILE__).'/composed.php';
require_once dirname(__FILE__).'/page.php';
require_once dirname(__FILE__).'/article.php';
require_once dirname(__FILE__).'/img.php';
require_once dirname(__FILE__).'/audio.php';
require_once dirname(__FILE__).'/video.php';
require_once dirname(__FILE__).'/embed.php';
require_once dirname(__FILE__).'/a.php';
require_once dirname(__FILE__).'/txt.php';
require_once dirname(__FILE__).'/category.php';
require_once dirname(__FILE__).'/cems.php';
require_once dirname(__FILE__).'/cliqued.php'
?>