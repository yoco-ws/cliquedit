<?php
namespace CE;class session{ public static function apiKey($API_key){\CE\session::$API_key=$API_key;session_start();} public static function loadEditor($debug=false){if(\CE\session::isEditMode()){\CE\session::initEditResources($debug);}else{echo '
			<style>
				label[data-ce-image]{
				    line-height: 0;
				    display: block;
				}
			</style>';}} private static function initEditResources($debug){if(!$debug){echo '<script src="//cdn.jsdelivr.net/npm/medium-editor@latest/dist/js/medium-editor.min.js"></script>';echo '<link rel="stylesheet" href="//cdn.jsdelivr.net/npm/medium-editor@latest/dist/css/medium-editor.min.css" type="text/css" media="screen" charset="utf-8">';echo '<link rel="stylesheet" href="css/ce.css">';echo '<script type="text/javascript" src="js/clic-edita.js"></script>';echo '
				<script>
			...
			';echo '<link rel="stylesheet" href="/clic-edita/src/client/css/ce.css">';echo '
				<script>
	    			var clicEdita = new ClicEdita(
						'.\CE\session::getKey().',
						"'.$_SERVER['HTTP_HOST'].'",
			...
			';}} public static function isEditMode(){if(isset($_SESSION['editmode'])){return true;}else{return false;}} public static function getKey(){return \CE\session::$API_key;} private static $API_key;}?>