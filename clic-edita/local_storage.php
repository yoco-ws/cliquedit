...
if(isset($_FILES["ce-image"]["type"])){
	...
	$path = $_SERVER["DOCUMENT_ROOT"].'/img/clic-edita/';
	$path_url= $root.'/img/clic-edita/';
	...
	if (
			(
				($_FILES["ce-image"]["type"] == "image/png") || ($_FILES["ce-image"]["type"] == "image/jpg") || ($_FILES["ce-image"]["type"] == "image/jpeg")
			) 
	...
	$path = $_SERVER["DOCUMENT_ROOT"].'/audio/clic-edita/';
	$path_url= $root.'/audio/clic-edita/';
	...
	if (
		...
		&& 	($_FILES["ce-audio"]["size"] < 10000000)//Approx. 100Mb files can be uploaded.
		&& in_array($file_extension, $validextensions)
	) {
	...
	$path = $_SERVER["DOCUMENT_ROOT"].'/video/clic-edita/';
	$path_url= $root.'/video/clic-edita/';
	...
	if (
		...
		&& 	($_FILES["ce-video"]["size"] < 10000000)//Approx. 100Mb files can be uploaded.
		&& in_array($file_extension, $validextensions)
	) {
